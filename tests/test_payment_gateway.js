...
async function runPaymentGatewayTests() {
  ...
  try {
    ...
    const adminUsername = 'admin_' + Math.random().toString(36).substring(2, 7)
    ...
    await fetch(`${baseUrl}/api/admin/promote`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-admin-key': process.env.ADMIN_SECRET_KEY || 'club69_admin_master_secret_2026',
      },
      body: JSON.stringify({ identity: adminUsername }),
    })
    process.env.ADMIN_IDENTIFIER = adminUsername

    const adminToken = generateToken({ id: adminSignupData.user.id, username: adminUsername, role: 'admin', is_admin: true })
    const adminHeaders = {
      'Content-Type': 'application/json',
    ...
    console.log(`✓ Test admin created & promoted: ${adminUsername} (ID: ${adminSignupData.user.id})`)
